...
def conductGrasslandAssessment(request):
    if request.method == "POST":
        ...
        farmer = Farmer(
            ...
            + " "
            + form["farmer_address_line_2"].value()
            + " "
...
def conductGrasslandAssessment2(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request, "conductGrasslandAssessment2.html", {"form": Grassland2()}
            )
...
def conductGrasslandAssessment3(request):
    if request.method == "POST":
        ...
        grass.date_taken = (
            date := parse(form["date_taken"].value(), dayfirst=True)
        ).strftime("%Y-%m-%d")
    ...
    return render(request, "conductGrasslandAssessment3.html", {"form": Grassland3})
...
def conductGrasslandAssessment4(request):
    if request.method == "POST":
        ...
        num_of_feed = Farmer_Feed(
            ...
            number_beat_pulps_molassed=(
                num6 := float(form["number_beat_pulps_molassed"].value())
            ),
            number_beat_pulp_unmolassed=(
                num7 := float(form["number_beat_pulp_unmolassed"].value())
            ),
            number_citrus_pulp=(num8 := float(form["number_citrus_pulp"].value())),
            number_maize_distiller=(
                num9 := float(form["number_maize_distiller"].value())
            ),
            ...
            number_distillers_grain=(
                num20 := float(form["number_distillers_grain"].value())
            ),
...
def importExportReport(request):
    ...
    Farmer.objects.filter(id=request.session.get("farmer_id")).update(is_assessed=True)
    return render(
        request, "importExportReport.html", {"list_for_result": list_for_result}
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render(
                request,
                "storage.html",
                {"form": storage_process, "farmer_list": farmer_list},
            )
        county_val = Farmer.objects.get(
            id=request.session.get("farmer_id")
        ).county.lower()
        ...
        slurry_vals = list(
            Monthly_Livestock_Numbers.objects.values_list("slurry_m3", flat=True)
        )
        manure_vals = list(
...
def storage_report(request):
    ...
    # The objects_to_update list will these columns in the database
    Slurry_Storage.objects.bulk_update(
        objects_to_update,
        ["total_slurry_manure", "total_storage", "max_storage", "space_available"],
    )
    Farmer.objects.filter(id=request.session.get("farmer_id")).update(is_assessed=True)
